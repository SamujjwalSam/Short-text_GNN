...
class ClassificationModel:
    def __init__(self, model_type="glen_xlmroberta", model_name="roberta-base",
        ...
        self.exp_name = exp_name + model_type + '_' + model_name +\
                        'num_labels_[' + str(args.num_labels) + ']_' +\
                        'num_epochs_[' + str(args.num_train_epochs) + ']_'

        logger.error(f'Experiment: {self.exp_name}')
        ...
        ## GLEN Layers:
        self.token_gcn = GCN(in_dim=self.args.in_dim, hid_dim=self.args.hid_dim,
    ...
    def forward(self, inputs, estimate_global_importance=False):
        ...
        if labels is not None:
            ...
            if self.args.multi_label and self.num_labels > 1:
                loss_fct = BCEWithLogitsLoss(pos_weight=self.weight)
                loss = loss_fct(
    ...
    def train(self, train_dataloader, output_dir, multi_label=cfg['data']['multi_label'],
        ...
        gcn_group = {}
        # gcn_group["lr"] = 0.001
        gcn_group["params"] = [p for n, p in self.token_gcn.named_parameters()]
        ...
        for _ in train_iterator:
            ...
            if args.evaluate_during_training and args.evaluate_each_epoch:
                ...
                for name, result in all_results.items():
                    logger.info(
                        f"Experiment at epoch [{epoch_number}] with [{self.exp_name}]"
                        f" for Data [{name}] W-F1 [{result['weighted_f1']}]:"
    ...
    def evaluate(self, eval_df, output_dir, multi_label=cfg['data']['multi_label'],
                 verbose=True, silent=False, wandb_log=True,
                 **kwargs):
        """
        ...
        result, wrong = self.compute_metrics(
            preds, model_outputs, out_label_ids, eval_examples,
            multi_label=multi_label, **kwargs
        )
