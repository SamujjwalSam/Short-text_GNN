...
# from Trainer.gcnr_trainer import GAT_BiLSTM_trainer
...
def create_glen_vocab(s_lab_df=None, data_dir: str = dataset_dir,
                      labelled_source_name: str = cfg['data']['name'],
...
def main(model_type='GNN', data_dir: str = dataset_dir, lr=cfg["model"]["optimizer"]["lr"],
    ...
    if exists(labelled_source_path + 'S_vocab.json')\
        ...
        T_dataset, T_fields, labelled_token2vec_map, s_lab_df =\
            create_glen_vocab(s_lab_df=None, data_dir=data_dir,
    ...
    if model_type == 'GAT':
        ...
        #     adj, X, train_dataloader, val_dataloader, test_dataloader,
        #     in_dim=cfg['embeddings']['emb_dim'], hid_dim=cfg['gnn_params']['hid_dim'],
        #     epoch=cfg['training']['num_epoch'], lr=lr, state=None)
    ...
    #     #     in_dim=cfg['embeddings']['emb_dim'], hid_dim=cfg['gnn_params']['hid_dim'],
    #     #     num_heads=cfg['gnn_params']['num_heads'], epoch=cfg['training']['num_epoch'], lr=lr, state=state)
    #
...
def prepare_datasets(
    ...
    if get_iter:
        ...
        train_iter, val_iter = dataset2bucket_dataloader(
...
def prepare_splitted_datasets(
    ...
    train_df = read_csv(data_file=train_dataname, data_dir=data_dir)
    ...
    if get_iter:
        ...
        test_batch_size = cfg['training']['eval_batch_size']
        train_iter, val_iter, test_iter = dataset2bucket_dataloader(
