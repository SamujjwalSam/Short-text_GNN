...
class InputExample(object):
    ...
    def __init__(
            self, text_a, text_b=None, label=None, x0=None, y0=None, x1=None,
            y1=None, guid=None
    ):
        """
...
def build_classification_dataset(
    ...
    if os.path.exists(cached_features_file) and (
        ...
        if len(data) == 4:
            logger.info("Preparing data for [Sentence pair task]")
            text_a, text_b, labels, idxs = data
        else:
            text_a, labels, idxs = data
            text_b = None
        ...
        labels = torch.tensor(labels)  ## Will convert to long inside loss
