...
def sscl_loss(prob, true_labels, hard_labels, soft_labels=None,
              ent_thresh=0.33, kd_loss_factor=1.):
    loss_fct = BCEWithLogitsLoss()
    loss = loss_fct(prob, true_labels)

    if soft_labels is not None:
        ## Calculate entropy for soft_labels
        uct = -soft_labels * torch.log(soft_labels + 1e-20)
        uct = torch.sum(uct, dim=1)
        uct_flag = uct < ent_thresh
        # logger.debug(torch.sum(uct_flag.long()))

        kd_loss = loss_fct(prob[uct_flag], hard_labels[uct_flag])
        # kd_loss = loss_fct(prob, hard_labels)

        loss = (kd_loss_factor * kd_loss) + ((1 - kd_loss_factor) * loss)

    if torch.isnan(loss) or torch.isinf(loss):
        logger.warn((loss, prob, true_labels, hard_labels, soft_labels))
...
class BertForMultiLabelSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        outputs = (logits,) + outputs[2:]  # add hidden states and attention

        if labels is not None:
            # if self.training:
            #     # print(labels.shape)
            #     loss = sscl_loss(prob=logits, true_labels=labels[:, 0].unsqueeze(1),
            #                      hard_labels=labels[:, 1].unsqueeze(1), soft_labels=labels[:, 2:4])
            # else:
            #     loss = sscl_loss(prob=logits, true_labels=labels.float(), hard_labels=None, soft_labels=None)
