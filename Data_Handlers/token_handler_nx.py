...
class Token_Dataset_nx(Dataset):
        ...
        if G is None:
            ...
            if exists(self.graph_path):
                ...
                if S_vocab is not None and T_vocab is not None:
                    self.G = self.create_glen_token_graph(corpus_toks, C_vocab,
                                                          S_vocab, T_vocab)
                else:
                    self.G = self.create_gcpt_token_graph(corpus_toks, C_vocab,
    ...
    def get_node_embeddings_from_dict(self, oov_embs: dict, embs: dict, i2s: list,
    ...
    def add_glen_edge_weights(self, alpha: float = 0.5, clear_data=False):
    ...
    def add_gcpt_edge_weights(self, clear_data: bool = False) -> nx.classes.graph.Graph:
        """ Calculate edge weight from occurrence values.

        :param clear_data: Removes occurrence and other attributes.
        :return:
        """
        for n1, n2, edge_data in self.G.edges(data=True):
    ...
    def renormalize_edge_weights_lpa(self, label_vectors: Tensor, eps=1e-9):
        """ Recalculate edge weight using LPA label vector similarity.

        NOTE: should be called after add_glen_edge_weights().
    ...
    def __getitem__(self, idx):
        assert idx == 0, "This dataset has only 1 large (token) graph."
        return self.G
    ...
    @staticmethod
    def create_glen_token_graph(datasets, c_vocab, s_vocab, t_vocab, window_size: int = 2):
    ...
    @staticmethod
    def create_gcpt_token_graph(dataset, c_vocab, window_size: int = 2):
